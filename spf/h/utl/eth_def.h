...
#define ETH_MAC_FMT   "%02x:%02x:%02x:%02x:%02x:%02x"
#define ETH_MAC_CHAR(m) (m)[0],(m)[1],(m)[2],(m)[3],(m)[4],(m)[5]
...
#define MAC_ADDR_MASK(_dst, _src, _mask) do { \
    (_dst)[0] = (_src)[0] & (_mask)[0]; \
    (_dst)[1] = (_src)[1] & (_mask)[1]; \
    (_dst)[2] = (_src)[2] & (_mask)[2]; \
    (_dst)[3] = (_src)[3] & (_mask)[3]; \
    (_dst)[4] = (_src)[4] & (_mask)[4]; \
    (_dst)[5] = (_src)[5] & (_mask)[5]; \
} while(0)

#define MAC_ADDR_COPY(_dst, _src) do {   \
    (_dst)[0] = (_src)[0];  \
    (_dst)[1] = (_src)[1];  \
    (_dst)[2] = (_src)[2];  \
    (_dst)[3] = (_src)[3];  \
